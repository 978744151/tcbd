...
define([
	...
	var AppRouter = Backbone.Router.extend({
		...
		routes: {
			...
			//跳转支付宝引导支付页面
			'jumpAlipay' : 'loadAlipayTips',

			//PK秀场
			'pkShow' : 'loadPkShow',

			//邀请奖励
			'invitAward(/:code)(/:name)' : 'loadInvitAwardShow',

			//PK分享
			'pkShare(/:roomid)(/:code)(/:auctionId)': 'loadPkShare',

			//中将分享
			'shareWinning(/:orderid)':'loadShareWinning',
			
			//邀请微信授权打开App
			'invitOauth(/:code)' : 'loadInvitOauth',
			
			//微信授权空页面
			'testPage(/:code)' : 'loadTestPage',
...
>>>>>>> Stashed changes
		...
		loadAlipayTips : function(){
			utils.hideMenu();
			requirejs(['views/alipayTips'], function (page) {
				if (!Views.alipayTips) {
					Views.alipayTips = new page();
				}
				Views.alipayTips.render();
			});
		},

		loadPkShow : function(){
			utils.hideMenu();
			requirejs(['views/pkShow'], function (page) {
				if (!Views.pkShow) {
					Views.pkShow = new page();
				}
				Views.pkShow.render();
			});
		},

		loadInvitAwardShow : function(code,name){
			utils.hideMenu();
			requirejs(['views/invitAward'], function (page) {
				if (!Views.invitAward) {
					Views.invitAward = new page();
				}
				Views.invitAward.render(code,name);
			});
		},

		loadPkShare : function(id,code){
			utils.hideMenu();
			requirejs(['views/pkShare'], function (page) {
				if (!Views.pkShare) {
					Views.pkShare = new page();
				}
				Views.pkShare.render(id,code);
			});
		},

		loadShareWinning : function(orderid){
			utils.hideMenu();
			requirejs(['views/shareWinning'], function (page) {
				if (!Views.shareWinning) {
					Views.shareWinning = new page();
				}
				Views.shareWinning.render(orderid);
			});
		},
		
		loadInvitOauth : function(code){
			utils.hideMenu();
			requirejs(['views/invitOauth'], function (page) {
				if (!Views.invitOauth) {
					Views.invitOauth = new page();
				}
				Views.invitOauth.render(code);
			});
		},
		loadTestPage : function(code){
			utils.hideMenu();
			requirejs(['views/testPage'], function (page) {
				if (!Views.testPage) {
					Views.testPage = new page();
				}
				Views.testPage.render(code);
			});
		}
