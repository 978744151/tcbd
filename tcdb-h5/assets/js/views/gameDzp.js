	...
	function ($, _, Backbone, UrlGroup, Swiper, echo, Api, map, kinerLottery, utils, scroll, Token,
		...
		var queryStatistics = function(){
			...
			Api.queryStatistics({
				...
				$page.find(".current-integration").html(data.result.points);
				$page.find(".ydb-limit").html(data.result.limit);
				$page.find(".ydb-lottery-count").html(data.result.lottery_count);
			}, function(data){
