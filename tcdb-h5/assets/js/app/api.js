define(
	...
	function ($, utils, _) {
		...
		return {
			...
			//同城夺宝微信授权
			wxOauthStep1: function (code, success) {
				$.wxAjax({
					url: '/wx-api/sns/oauth2/access_token?appid=wx1f04a1d3d7493132&secret=e4b631bc2107626dd3c17b7fb57ff53a&code=' + code + '&grant_type=authorization_code',
					type: "GET",
					dataType: 'json',
					success: function (data) {
						typeof success == 'function' && success(data);
			...
			wxOauthStep2: function (token, openId, success) {
				$.wxAjax({
					url: '/wx-api/sns/userinfo?access_token=' + token + '&openid=' + openId + '&lang=zh_CN',
					type: "GET",
					dataType: 'json',
					success: function (data) {
						typeof success == 'function' && success(data);
			...
			//同城夺宝绑定邀请关系
			inviteBind: function (invitation_code, unionid, nickname, avatar, success, error) {
				$.ajax({
					url: window.API_URL + '-api/invitation/bind',
					type: "POST",
					data: {
